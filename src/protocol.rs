...
pub const PLAN_FORMAT: &str = "ores.migration-plan/v2";
...
impl MigrationPlan {
    /// Decodes and validates the published v2 migration-plan contract.
    ///
    /// # Errors
    ///
    ...
    /// Validates cross-field invariants that JSON Schema cannot express alone.
    ///
    /// # Errors
    ///
    /// Returns an [`InterfaceError`] when the plan is ambiguous, unsafe, or
    ...
    pub fn validate(&self) -> Result<(), InterfaceError> {
        ...
        if self.phases.is_empty() {
            return Err(InterfaceError::EmptyPhases);
        }
        ...
        for phase in &self.phases {
            ...
            if !phase_ids.insert(metadata.id.as_str()) {
                return Err(InterfaceError::DuplicatePhaseId);
            }
...
impl MigrationPhase {
    ...
    fn validate(&self, engine: DatabaseEngine) -> Result<(), InterfaceError> {
        match self {
...
fn validate_statements(statements: &[MigrationStatement]) -> Result<(), InterfaceError> {
    ...
    for (index, statement) in statements.iter().enumerate() {
        let expected = u32::try_from(index + 1)
            .map_err(|_| InterfaceError::InvalidPhase("statement count exceeds u32"))?;
...
fn validate_checks(checks: &[MigrationCheck]) -> Result<(), InterfaceError> {
    for check in checks {
...
fn require_non_empty(value: &str, field: &'static str) -> Result<(), InterfaceError> {
    if value.trim().is_empty() {
        Err(InterfaceError::EmptyField(field))
    } else {
        Ok(())
    }
}

fn require_sha256(value: &str, field: &'static str) -> Result<(), InterfaceError> {
    if value.len() == 64
        && value
            .bytes()
            .all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
    {
        ...
        Err(InterfaceError::InvalidSha256(field))
    }
}
...
mod tests {
    ...
    fn round_trips_a_typed_plan() {
        ...
        let actual = MigrationPlan::from_json(&json).expect("parse typed plan");
    ...
    fn rejects_the_legacy_opaque_payload_shape() {
        ...
        assert_eq!(
            MigrationPlan::from_json(legacy),
    ...
    fn rejects_duplicate_phase_ids() {
        ...
        assert_eq!(plan.validate(), Err(InterfaceError::DuplicatePhaseId));
    }
    ...
    fn rejects_destructive_changes_outside_cleanup() {
        ...
        assert_eq!(
            plan.validate(),
            Err(InterfaceError::InvalidPhase(
                "destructive changes must use destructive_cleanup"
            ))
        );
    }
    ...
    fn rejects_non_lowercase_sha256() {
        ...
        assert_eq!(
            plan.validate(),
            Err(InterfaceError::InvalidSha256("source_catalog_sha256"))
        );
    }
}
